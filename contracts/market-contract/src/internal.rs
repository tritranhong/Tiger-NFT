...
impl Contract {
    ...
    pub(crate) fn internal_remove_auction(&mut self, nft_contract_id: AccountId, token_id: TokenId) -> Auction {
        ...
        let mut by_owner_id = self.by_owner_id.get(&auction.owner_id).expect("Not found auction by owner_id");
        by_owner_id.remove(&contract_and_token_id);
    
        if by_owner_id.is_empty() {
            self.by_owner_id.remove(&auction.owner_id);
        } else {
            self.by_owner_id.insert(&auction.owner_id, &by_owner_id);
        }

        let mut by_contract_id = self.by_contract_id.get(&nft_contract_id).expect("Not found auction by contract_id");
        by_contract_id.remove(&token_id);
        if by_contract_id.is_empty() {
            self.by_contract_id.remove(&nft_contract_id);
        } else {
